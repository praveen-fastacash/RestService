...
public class GcdSOAPIT {
	...
	{
		String url = "http://localhost:" + localServerPort +"/soapservice/gcd?wsdl";
		
		GcdPort port = new GcdService(new URL(url)).getGcd();
		...
		response = port.retrieveFirst(request);
		Assert.notNull(response.getFirstGcd(), "Expected values not present");
	}
